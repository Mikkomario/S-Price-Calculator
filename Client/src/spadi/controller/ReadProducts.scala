...
import spadi.model.{DataSource, ProductBasePrice, ProductPrice, ProgressState, SalesGroup, ShopSetup}
...
object ReadProducts
{
			...
			_.lastModified.toOption.forall { _ > lastReadTime } } } || files.exists { !FileReads.contains(_) })
		{
			...
			val result = setupsToUse.map { setup =>
				val readData = setup.dataSource
					.mapBoth { case (baseSource, saleSource) =>
						read(baseSource, progressPointer, progressPerFile)
							.flatMap { base => read(saleSource, progressPointer, progressPerFile)
								.map { sale => base -> sale } }
					} { comboSource =>
						read(comboSource)
					}.mapToSingle {
						_.map[Either[(Vector[ProductBasePrice], Vector[SalesGroup]), Vector[ProductPrice]]] { Left(_) } } {
						_.map { Right(_) } }
				setup.shop -> readData
			}
	...
	private def read[A](source: DataSource[A], progressPointer: PointerWithEvents[ProgressState], progressAdvance: Double) =
	{
		...
		val result = ReadExcel.from(source.filePath, target).flatMap { models =>
