...
object ReadExcel
{
		...
		rowsFromSheet: (Sheet, SheetTarget) => Vector[Row]) =
	{
		...
		targets.flatMap
		{ case (sheetKey, target) =>
			// Parses row data from the sheets
			val parsedData = target.sheetId.mapToSingle
			{ name => Option(workBook.getSheet(name)) }
			{ idx =>
				if (idx < numberOfSheets) Some(workBook.getSheetAt(idx))
				else None
			}.map
			{rowsFromSheet(_, target)}
			
			// Produces map format
			parsedData.map
			{ models => sheetKey -> models }
		}
	}
	...
	{
		parseWorkBookWith(workBook, targets)
		{ (sheet, target) =>
			...
			if (findHeadersIterator.hasNext)
			{
				...
				val headers =
					{
						limitedIterator(headersRow.cellIterator().asScala, target.firstCellIndex, target.maxCellsRead).map
						{ cell =>
						  ...
						  maxCellsRead: Option[Int] = None) =
	{
		...
		val rowsIterator =
		{
			...
			maxRowsRead.map
			{ max => base.take(max) }.getOrElse(base)
		}
		rowsIterator.map
		{ row =>
			val cellValues = limitedIterator(row.cellIterator().asScala, firstCellIndex, maxCellsRead).map
			{ cell =>
			...
			(firstCellIndex to lastCellIndex).map
			{ idx => cellValues.getOrElse(idx, Value.empty) }.toVector
		}.toVector
	}
	
	private def limitedIterator(cellIterator: Iterator[Cell], firstCellIndex: Int = 0, maxCellsRead: Option[Int] = None) =
	{
		val base = cellIterator.dropWhile
		{_.getColumnIndex < firstCellIndex}
		maxCellsRead.map
		{ max => base.takeWhile
		{_.getColumnIndex < firstCellIndex + max}
		}.getOrElse(base)
	}
