...
object ReadProducts2
{
	...
	private val supportedFileTypes = ReadExcel.supportedFileTypes.map { _.toLowerCase } :+ "csv"
	
	
	// COMPUTED ------------------------------
	
	private def targetFiles = inputDirectory.findDescendants { file =>
		file.isRegularFile && supportedFileTypes.contains(file.fileType.toLowerCase)
	}
