...
import spadi.view.controller.{MainVC2, NewFileConfigurationUI2}
...
object SPriceApp2 extends App
{
	...
	DbSetup.setup(dbSetupProgressPointer) match
	{
		case Success(dbVersion) =>
			...
			NewFileConfigurationUI2.readAndConfigureBlocking()
			...
			} match
			{
				case Success((shops, defaultProducts)) =>
					...
					val frame = Frame.windowed(new MainVC2(shops, defaultProducts), "S-Padi Hintalaskuri",
