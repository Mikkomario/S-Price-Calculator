...
import spadi.view.component.{ProductsView2, SearchField}
 ...
 * @since 9.5.2020, v1
 */
class MainVC2(shops: Iterable[Shop], defaultProducts: Vector[Product])
	extends StackableAwtComponentWrapperWrapper with AwtContainerRelated
{
	...
	private val (searchField, view) = baseContext.inContextWithBackground(colorScheme.primary).use { implicit c =>
		...
		val productsView = ProductsView2(productsPointer, shops, Screen.height / 2)
