 ...
 */
object NewFileConfigurationUI2
{
	// ATTRIBUTES   ------------------------
	...
	def configureBlocking(target: Either[Vector[Path], Vector[FileReadSetting2]]): Unit =
	{
		// Displays a settings dialog for the new paths
		val readSettingsFrame = new FileReadSettingsFrame2(target)
	...
	private class DataSourceInput[A, M <: KeyMapping[A]](val setting: FileReadSetting2,
														 mappingFactory: KeyMappingFactory2[A, M])
