...
object FileReadSettingsFrame2
{
	/**
	  ...
	  * @return A new frame
	  */
	def forAdding(paths: Vector[Path]) = new FileReadSettingsFrame2(Left(paths))
	...
	/**
	  ...
	  * @return A new frame
	  */
	def forEditing(settings: Vector[FileReadSetting2]) = new FileReadSettingsFrame2(Right(settings))
}
 ...
 */
class FileReadSettingsFrame2(base: Either[Vector[Path], Vector[FileReadSetting2]]) extends Area
{
	// ATTRIBUTES   -----------------------------------
	
	private var output: Vector[FileReadSetting2] = Vector()
	...
	private def removeRow(rowToRemove: FileReadSettingInputRow2): Unit =
	{
